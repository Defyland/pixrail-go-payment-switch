...
| `go test -bench=. -benchmem ./internal/api` | Passed | `BenchmarkCreateTransfer-10 46824 24465 ns/op 28464 B/op 230 allocs/op`. |
...
| `go run ./cmd/pixrail-api` plus HTTP smoke | Passed | `/readyz` returned store-backed ready, `POST /v1/pix/transfers` returned `201`, and authenticated `/v1/outbox` returned five payment-rail events. |
